...
contract LotteryProject_GradeA_Nakub {
    ...
    // function getWinnersByRound ฟังก์ชันเพื่อดูผู้ชนะในแต่ละรอบ
       function getWinnersByRound(uint round) public view returns (string memory) {
        require(round > 0 && round <= _roundsPlayed, "Invalid round number");                               // ตรวจสอบ รอบ ถ้ารอบไม่ถูกต้อง
        uint index = round - 1;                                                                             // หารอบที่ต้องการดู (รอบเริ่มจาก 1, แต่ index ของ array เริ่มจาก 0)
        address payable[] memory winners = _winnersByRound[index];                                          // ดึงอาร์เรย์ของผู้ชนะในรอบนั้น
        string memory result = "";                                                                          // ตัวแปรเก็บผลลัพธ์เริ่มต้นเป็นสตริงว่าง
        for (uint i = 0; i < winners.length; i++) {                                                         // ลูปผ่านอาร์เรย์ของผู้ชนะ
            result = string(abi.encodePacked(result, "address: ", toAsciiString(winners[i]), ", "));        // รวมที่อยู่ผู้ชนะเข้าในสตริงผลลัพธ์
        }
        return result;                                                                                      // Return สตริงผลลัพธ์
    }

    function toAsciiString(address x) internal pure returns (string memory) {
    bytes memory s = new bytes(42);                                                                         // สร้างอาร์เรย์ของ bytes ขนาด 42
    s[0] = '0';                                                                                             // ใส่ '0' ที่ตำแหน่งแรก
    s[1] = 'x';                                                                                             // ใส่ 'x' ที่ตำแหน่งที่สอง
    for (uint i = 0; i < 20; i++) {                                                                         // ลูปผ่าน 20 bytes ของที่อยู่
        bytes1 b = bytes1(uint8(uint(uint160(x)) / (2**(8*(19 - i)))));                                     // ดึง byte แต่ละตัวจากที่อยู่
        bytes1 hi = bytes1(uint8(b) / 16);                                                                  // แปลง byte เป็นตัวเลขฐาน 16 ส่วนสูง
        bytes1 lo = bytes1(uint8(b) - 16 * uint8(hi));                                                      // แปลง byte เป็นตัวเลขฐาน 16 ส่วนน้อย
        s[2*i + 2] = char(hi);                                                                              // ใส่ตัวเลขฐาน 16 ส่วนสูงในอาร์เรย์
        s[2*i + 3] = char(lo);                                                                              // ใส่ตัวเลขฐาน 16 ส่วนน้อยในอาร์เรย์
    }
    return string(s);                                                                                       // คืนค่าอาร์เรย์ bytes เป็นสตริง
    }


    function char(bytes1 b) internal pure returns (bytes1 c) {
        if (uint8(b) < 10) return bytes1(uint8(b) + 0x30);                                                  // ถ้า byte น้อยกว่า 10 ให้แปลงเป็นตัวเลข
        else return bytes1(uint8(b) + 0x57);                                                                // ถ้า byte มากกว่าหรือเท่ากับ 10 ให้แปลงเป็นตัวอักษร
    }
